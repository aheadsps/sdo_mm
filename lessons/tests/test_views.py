...
from lessons.models import TestBlock, Lesson, Course
...
class TestBlockAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.lesson_data = {
            'name': 'Lesson 1',
            'serial': 1,
            'course': Course.objects.create(name='Course 1').pk  # Создаем связанный курс
        }
        self.lesson = Lesson.objects.create(**self.lesson_data)
        self.test_block_data = {
            'lessons': self.lesson.pk
        }
    ...
    def test_update_test_block(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(TestBlock.objects.get(pk=self.test_block.pk).lessons_id, self.lesson.pk + 1)
